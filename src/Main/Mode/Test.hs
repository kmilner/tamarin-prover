...
import Test.VerdictTests qualified as Verdict (tests)
...
run _thisMode as = do
  ...
  nextTopic "Testing proof verdict invariants"
  successVerdict <- runUnitTest =<< Verdict.tests (maudePath as)
  ...
  let success = successMaude && successGraphVizDot && successTerm && successVerdict
  if success then do
