...
import Main.TheoryLoader (TheoryLoadError(ExportTranslationError, ParserError, UnsupportedInputError, WarningError), TheoryLoadOptions(..))
import Theory
  ...
  where
    loadThy (idx, path) = do
      ...
      case result of
        ...
        Left (UnsupportedInputError report) -> do
          -- Skip this theory and keep serving the others.
          putStrLn $ renderDoc $ ppInteractive report path
          putStrLn $ "Rejected " ++ path ++ ": unsupported rule semantics."
          pure Nothing
