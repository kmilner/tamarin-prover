...
/// (`rust/crates/tamarin-server/`).  Used to locate the
/// shared `data/` directory.
pub fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is `<repo>/rust/crates/tamarin-server`
    let mf = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    mf.parent() // crates
        .and_then(|p| p.parent()) // rust
        .and_then(|p| p.parent()) // repo root
        .expect("workspace root above rust/crates/tamarin-server/")
        .to_path_buf()
}
...
pub async fn start_server_with_theory(fixture_name: &str) -> TestServer {
    ...
    let data_dir = resolve_data_dir(Some(workspace_root().join("data")));
