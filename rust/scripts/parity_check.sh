...
HS_BIN="${HS_BIN:-$repo/.stack-work/install/x86_64-linux-tinfo6/ec0cb11b1bfcf8776d45e0357bbc6d6ff2077f9222735af22115429c8cdfcef1/9.6.7/bin/tamarin-prover}"
RS_BIN="${RS_BIN:-$repo/rust/target/release/tamarin-prover}"
TIMEOUT="${TIMEOUT:-120}"
prove_arg="--prove"
...
resolve() {
  ...
  [ -f "$repo/examples/$f" ] && { echo "$repo/examples/$f"; return; }
  local hit; hit="$(find "$repo/examples" -name "$(basename "$f")" 2>/dev/null | head -1)"
