...
def main():
    if len(sys.argv) < 3:
        print("usage: web_crawl.py BASE OUT.json [--max-nodes N]", file=sys.stderr)
        sys.exit(2)
    ...
    idx = 1
    # 1. discover lemmas from theory-1 overview
    _, _, ov = http_get(base, f"/thy/trace/{idx}/overview/help")
    lemmas = []
    for h in hrefs(ov):
    ...
    # 2. static routes
    for r in ["overview/help", "source", "message", "main/rules", "main/help",
