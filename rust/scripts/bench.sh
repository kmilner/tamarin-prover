...
#
#   rust/scripts/bench.sh            # measure, print the block to stdout
#   rust/scripts/bench.sh --write    # measure, then rewrite the block IN PLACE
#                                     #   between the markers in rust/README.md
#
# Env knobs:
...
#   README_PATH  file to rewrite in --write mode        (default: rust/README.md)
#
# Methodology:
...
README_PATH="${README_PATH:-$repo_root/rust/README.md}"
TIMEOUT="${TIMEOUT:-600}"
DERIV="${DERIV:-30}"
CORES="${CORES:-1 4 16}"
...
HDR
    ...
    for k in $CORES; do
        ...
        echo
        echo "| Theory | HS time | RS time | HS memory | RS memory |"
        echo "|--------|--------:|--------:|----------:|----------:|"
        for f in $FILES; do
            af="$CORPUS/$f"
