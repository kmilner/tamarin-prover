...
#      HS_PORT (3021), RS_PORT (3022), CORPUS_ROOT (examples/), ALLOWLIST
...
find_hs_bin() {
    local c
    for c in "$repo_root"/.stack-work/install/*/*/*/bin/tamarin-prover; do
        [ -x "$c" ] && { echo "$c"; return 0; }
    done; return 1
}
...
if [ -z "${TAM_RS_NO_AUTO_BUILD:-}" ]; then
    ...
    ( cd "$repo_root/rust" && cargo build --release -q -p tamarin-prover ) || {
...
boot_crawl() {
    ...
    setsid "$bin" interactive "$wd/thy" --port="$port" >"$log" 2>&1 &
    pid=$!
    # wait for readiness
    local ok="" i
    ...
    timeout "$FILE_TIMEOUT" python3 "$script_dir/web_crawl.py" \
        "http://127.0.0.1:$port" "$out" --max-nodes "$MAX_NODES" 2>>"$log"
    local rc=$?
    kill -- -"$pid" 2>/dev/null; wait "$pid" 2>/dev/null
    return $rc
}
