...
-- | Build a successor-adjacency map from a relation. Computing this once and
-- reusing it across many reachability queries (see 'reachableSetWith') avoids
-- rescanning the whole relation per query. The order of successors per key is
-- unspecified, which is fine for the order-independent consumers below.
...
-- prebuilt adjacency map (see 'adjacency'). Prefer this over 'reachableSet'
-- whenever the same relation is queried repeatedly (e.g. 'alwaysBefore').
...
-- | Compute the set of nodes reachable from the given set of nodes. Builds the
-- adjacency map for a single use; use 'reachableSetWith' for repeated queries.
