      ...
      (case partitionEithers $ mapMaybe (findRule funsym) (S.toList rules) of
        ...
        ([], equations0) -> do
          ...
          where equations = renameAvoiding equations0 avoidTerms
                ...
                new_an = annDestructorEquation translatedEquations elsebranch
          )
...
translateLetDestr rules p = mapProc varTermsToAvoid rules p
  where
    varTermsToAvoid = [varTerm v | SapicLVar v _ <- S.toList $ varsProc p]
