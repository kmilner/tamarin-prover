...
import Control.Monad.State.Strict qualified as MS
...
-- | Add source actions to members without detaching them from their parents.
-- The ordinary generator preserves rule order and count. Consume its updated
-- members in that order, retaining owned families (including empty sides).
addAutoSourcesLemmaDiff :: MaudeHandle -> String -> ClosedRuleCache -> ClosedRuleCache
  -> [DiffTheoryItem ClosedDiffRule ClosedRuleFamily IncrementalDiffProof IncrementalProof]
  -> [DiffTheoryItem ClosedDiffRule ClosedRuleFamily IncrementalDiffProof IncrementalProof]
addAutoSourcesLemmaDiff hnd lemmaName crcLeft crcRight items =
    owned ++ sideItems LHS lhs ++ sideItems RHS rhs
  where
    lhs = generate LHS crcLeft
    rhs = generate RHS crcRight
    generate side cache =
      let input = concatMap (onSide side) items
      in if containsPartialDeconstructions cache
         then addAutoSourcesLemma hnd (lemmaName ++ "_" ++ show side) cache input
         else input
    onSide side item = map RuleItem (diffItemSideRules side item) ++ case item of
      EitherLemmaItem (s,l) | s == side -> [LemmaItem l]
      EitherRestrictionItem (s,r) | s == side -> [RestrictionItem r]
      _ -> []
    rules = mapMaybe itemToRule
    owned = case MS.runState (traverse (traverseDiffRuleMembers replace) (filter keep items))
                   (rules lhs, rules rhs) of
      (updated, ([],[])) -> updated
      _ -> error "Auto-sources changed a diff family's member count"
    replace :: Side -> ProtoRuleE -> ProtoRuleAC -> MS.State ([ClosedProtoRule], [ClosedProtoRule]) [ProtoRuleAC]
    replace side parent _ = MS.state $ \(ls,rs) ->
      let takeMember (ClosedProtoRule e ac:rest)
            | e == parent = ([ac], rest)
          takeMember _ = error "Auto-sources changed a diff family's member order"
      in case side of
        LHS -> let (member,rest) = takeMember ls in (member,(rest,rs))
        RHS -> let (member,rest) = takeMember rs in (member,(ls,rest))
    keep (EitherLemmaItem _) = False
    keep (EitherRestrictionItem _) = False
    keep (DiffMacroItem _) = False
    keep _ = True
    sideItems side = mapMaybe $ \item -> case item of
      LemmaItem l -> Just $ EitherLemmaItem (side, if side == LHS then addLeftLemma l else addRightLemma l)
      RestrictionItem r -> Just $ EitherRestrictionItem (side,r)
      TextItem t -> Just $ DiffTextItem t
      MacroItem m -> Just $ DiffMacroItem m
      ConfigBlockItem b -> Just $ DiffConfigBlockItem b
      _ -> Nothing
