...
-- | Fact tags/symbols
data FactTag = ProtoFact Multiplicity String [Bool]
               -- ^ A protocol fact together with its multiplicity and invariant terms
...
protoFact multi name ts = 
    Fact (ProtoFact multi name (replicate (length ts) False)) ts
