...
data FactTag = ProtoFact Multiplicity String Int
             ...
             | InvFact String Int
               -- ^ An invariant fact is a linear fact where, after application
               -- of a rule that contains it, its term arguments do not change.
    deriving( Eq, Ord, Show, Typeable, Data, Generic, NFData, Binary )
-- | Facts.
data Fact t = Fact
    { factTag   :: FactTag
...
-- | An invariant fact is similar to a linear protocol fact but all rules that
-- contain it either introduce it to or remove it from the multiset, or keep
-- the fact's term arguments constant.
invariantFact :: String -> [t] -> Fact t
invariantFact name ts = Fact (InvFact name (length ts)) ts
...
-- | True if the fact is an invariant fact.
isInvariantFact :: Fact t -> Bool
isInvariantFact (Fact (InvFact {}) _) = True
isInvariantFact _                     = False
...
-- | Convert a protocol fact into an invariant fact.
protoToInvFact :: LNFact -> Maybe [Int] -> LNFact
protoToInvFact (Fact (ProtoFact Linear n _) ts) (Just is) = invariantFact n $ (ts !!) <$> is
protoToInvFact (Fact (ProtoFact Linear n _) ts) Nothing   = invariantFact n ts
protoToInvFact (Fact tag _) _                             = error $ "protoToInvFact: Cannot convert the fact " ++ (showFactTag tag) ++ " into an invariant fact"
