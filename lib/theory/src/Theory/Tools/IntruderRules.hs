...
destructionRules _    (StRule lhs@(viewTerm -> FApp (NoEq (NoEqSym f _ _ _ _)) _) (RhsPosition pos)) =
    go [] lhs pos
  where
    rhs = lhs `atPos` pos
    go _      _                       []     = []
...
destructionRules _    (StRule (viewTerm -> FApp (NoEq (NoEqSym f _ _ _ _)) subterms) (RhsGround rhs@(viewTerm -> FApp (NoEq (NoEqSym _ 0 Private _ _)) []))) = destrRulesForConstant subterms f rhs
destructionRules True (StRule (viewTerm -> FApp (NoEq (NoEqSym f _ _ _ _)) subterms) (RhsGround rhs@(viewTerm -> FApp (NoEq (NoEqSym _ 0 Public _ _))  []))) = destrRulesForConstant subterms f rhs
...
privateConstructorEquations rs = case rs of
    []    -> []
    (StRule lhs (RhsGround (viewTerm -> FApp (NoEq (NoEqSym vname 0 Private _ _)) []))):xs
...
privateConstructorRules rules = map createRule $ derivablePrivateConstants (privateConstructorEquations rules) []
  where
    ...
    createRule s = Rule (ConstrRule s) [] [concfact] [concfact]
        where m        = fAppNoEq (NoEqSym s 0 Private Nothing False) []
              concfact = kuFact m
